...
- [x] DCIM documentation rewrite (DCIM.md: compressor, MB-XNOR, C pseudo-code, OpenLane hardening, performance estimates)
- [x] Create `tinymoa_compressor` module (double-approx, single-approx, exact via ifdef)
- [x] Fix dcim.v default addresses (0x1A0/0x1C0/0x1E0), bit-plane processing order (MSB-first)
- [x] Move DCIM files to `src/dcim/` directory
- [x] Fix `registers.v:54`: `rd_wr_en` -> `rd_wen`
- [x] Fix `alu.v`: added SUB (`4'b1000` -> `sum[3:0]`), CZERO.EQZ/NEZ pass-through (`a_in`); SLT/SLTU handled by `cmp_out` combinational logic (default `result=0` is correct)
- [x] Fix `Architecture.md` module hierarchy: `sram.v` -> `tcm.v`, `boot.v` -> `bootloader.v`; fix default address constants
- [x] Implement DCIM FSM: pipelined LOAD_WEIGHTS (N+1 cycles), FETCH_ACT (2-cycle with fetch_wait), COMPUTE (shift-accumulate), STORE_RESULT (signed conversion), DONE
- [x] Instantiate 2x `tinymoa_compressor` per column, sum -> `popcount[col]`
- [x] Hardware signed conversion: `store_signed = 2*shift_acc[col] - bias_reg`, sign-extended to 32-bit, 17-bit intermediate
- [x] Fix dcim.v signed conversion negative repeat: widened `store_signed` from `ACC_WIDTH+2` to 17 bits
- [x] Fix `tinymoa.v` trailing comma in `tinymoa_cpu` instantiation
- [x] Set up `tb_dcim.v` (integration: with behavioral TCM; unit: exposed memory bus)
- [x] Test runner: added `defines=["BEHAVIORAL"]` for TCM simulation, fixed `dcim/dcim.v` path, added missing `bootloader.v`
- [x] Updated README.md (root, src/, test/)
...
Nibble-serial adder, comparison logic, CZERO, shifter, and multiplier are implemented. Remaining:
